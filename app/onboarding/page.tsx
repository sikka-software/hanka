  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-neutral-950">
      <div className="border border-neutral-800 rounded-xl p-8 bg-neutral-900 w-full max-w-md">
        <h1 className="text-2xl font-bold text-neutral-50 mb-2">Set up your skill library</h1>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="w-full mb-6">
            ...
            <TabsTrigger value="existing" className="flex-1">Use existing</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="create" className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-xs text-neutral-500">Only you can access this repo</p>
              </div>
              <Switch
                id="isPrivate"
                checked={isPrivate}
            ...
            {!isPrivate && username && (
              <p className="text-xs text-neutral-400 bg-neutral-800 p-3 rounded-lg">
                ...
                <code className="text-neutral-300">npx skills add {username}/{repoName} --skill skillname</code>
              </p>
            )}
            <Button onClick={handleCreate} disabled={loading || !repoName} className="w-full">
            ...
            {fetchingRepos ? (
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {repos.map(repo => (
                  ...
                  >
                    <div className="font-medium text-neutral-100">{repo.name}</div>
                    {repo.description && (
                      <div className="text-sm text-neutral-400 truncate">{repo.description}</div>
                    )}
                    <div className="text-xs text-neutral-500 mt-1">
                      {repo.private ? 'Private' : 'Public'} · Updated {new Date(repo.updatedAt).toLocaleDateString()}
                    </div>
                  </button>
                ))}
              </div>
