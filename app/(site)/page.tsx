...
const features = [
  {
    ...
    title: "GitHub-backed",
    description:
      "Every skill is a markdown file in your own GitHub repo. You own your data.",
  },
  {
    icon: Lock,
    title: "Private repo support",
    description:
      "Keep your skills private. CLI authenticates via GitHub Device Flow.",
  },
  {
    icon: Terminal,
    title: "CLI-first",
    description: "Use Vercel's skills CLI to pull any skill in seconds.",
  },
];
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-[0.015]">
      ...
      <div className="absolute top-[40%] right-[8%] font-mono text-xs leading-relaxed text-orange-500/60">
        <div>{`// Using Vercel skills CLI`}</div>
        <div>{`$ skills add username/repo`}</div>
        <div className="pl-4">{`// → skill added!`}</div>
      </div>
      <div className="absolute bottom-[25%] left-[12%] font-mono text-xs leading-relaxed text-amber-600/50">
        <div>{`$ skills add \\`}</div>
        <div className="pl-4">{`  sikka-software/hanka \\`}</div>
        <div className="pl-4">{`  --skill expert-coder`}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <section className="relative min-h-[85vh] flex flex-col items-center justify-center px-6 py-32">
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        >
          Publish your LLM skill files as a GitHub repo. Use{" "}
          <span className="text-amber-400 font-mono text-sm">skills add</span>{" "}
          to pull them in.
...
function CLIDemo() {
  ...
  const codeLines = [
    // { text: "# Install Vercel's skills CLI", color: "text-neutral-500" },
    // { text: "$ npm install -g @vercel/ai-cli", color: "text-amber-400" },
    { text: "", color: "" },
    {
      text: "# Add a skill from your skills collection",
      color: "text-neutral-500",
    },
    {
      text: "npx skills add your-username/your-collection-repo --skill skill-name",
      color: "text-neutral-200",
    },
    { text: "", color: "" },
    { text: "✓ Skill added successfully!", color: "text-green-400" },
    { text: "✓ Dependencies resolved", color: "text-green-400" },
  ];
    ...
    <main className="min-h-screen relative">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 backdrop-blur-xl bg-neutral-950/50 border-b border-neutral-800/30">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            variant="outline"
            className="border-neutral-700 hover:border-amber-500/50"
          >
